...
# Luminote: Comprehensive Design Document<a name="luminote-comprehensive-design-document"></a>

This document provides the complete design specifications for Luminote,
including purpose, roadmap, and detailed features for all development phases.
...
## Purpose & Overview<a name="purpose--overview"></a>

### What is Luminote?<a name="what-is-luminote"></a>

Luminote is a dual-pane translation workbench that makes dense web content
understandable, verifiable, and reviewable. It keeps translation as the primary
view while offering on-demand AI assistance for insights and verification.

### Who It Serves<a name="who-it-serves"></a>

- Readers of high-density materials (news, research papers, technical docs,
  financial reports)
...
### Core Outcomes<a name="core-outcomes"></a>

- Faster comprehension of source content through progressive, block-level
  translation
- Greater confidence via optional verification and provenance of AI outputs
- Reduced friction with reader-mode extraction and synchronized dual-pane
  navigation

### Key Differentiators<a name="key-differentiators"></a>
...
- **On-demand AI**: User-controlled cost with explicit triggers for all AI
  operations
- **BYOK multi-provider**: Support OpenAI, Anthropic, and other providers with
  user's own API keys
...
### Phase 1: Core Translation Workbench (Foundation)<a name="phase-1-core-translation-workbench-foundation"></a>

Establish Luminote as a reliable, stable translation reader with essential
features for daily use.

**Delivery Features:**

- Dual-pane translation with progressive, block-level rendering
- Reader-mode extraction from most public URLs (titles, paragraphs, lists,
  quotes, code, images)
...
### Phase 2: Enhanced Translation & Versioning<a name="phase-2-enhanced-translation--versioning"></a>
...
- Local visit history and quick paste-text translation (fallback when extraction
  fails)
- Selection-based commands: Explain, Define terms, Summarize (save as notes)
- Prompt templates, termbase setup, translation versioning (keep recent
  versions)

### Phase 3: Verification & Knowledge Integration<a name="phase-3-verification--knowledge-integration"></a>
...
# Phase 1: Core Translation Workbench Features<a name="phase-1-core-translation-workbench-features"></a>

This phase establishes Luminote as a stable, reliable translation workbench.

______________________________________________________________________

## 1. Dual-Pane Translation with Progressive, Block-Level Rendering<a name="1-dual-pane-translation-with-progressive-block-level-rendering"></a>

### Overview<a name="overview"></a>

Display source and translated content side-by-side with intelligent block-level
synchronization, enabling fast comprehension without losing context.
...
## 2. Reader-Mode Extraction from Most Public URLs<a name="2-reader-mode-extraction-from-most-public-urls"></a>

### Overview<a name="overview-1"></a>

Intelligently extract and clean main content from web pages, removing clutter
while preserving structure, images, and semantic meaning.

### Key Requirements<a name="key-requirements-1"></a>
...
- **Research papers/PDFs**: Title, abstract, sections, figures, tables,
  references
...
- **Remove**: Navigation menus, footers, ads, tracking pixels, popups, "cookie
  consent" banners
...
- **JavaScript-heavy sites**: Use headless browser (Playwright/Selenium) if
  initial HTML extraction yields \<30% content
- **Paywalled content**: Detect and skip; notify user with clear message
- **Non-HTML (images, videos)**: Graceful fallback—provide URL and request
  manual paste-text mode
- **Very long pages**: Chunk extraction into sections; auto-paginate or truncate
...
## 3. BYOK (Bring Your Own Key) Configuration<a name="3-byok-bring-your-own-key-configuration"></a>

### Overview<a name="overview-2"></a>

Users provide their own API credentials, ensuring full privacy, cost control,
and provider flexibility. No centralized credential storage.

### Key Requirements<a name="key-requirements-2"></a>
...
- **Browser-only storage**: default to session-only memory; allow opt-in
  persistence in localStorage or IndexedDB (client-side encryption optional)
...
- Allow partial config (e.g., remember provider+language, but re-enter key each
  session)
...
## 4. Clear Error Handling for Fetch Failures, Invalid Keys, and Rate Limits<a name="4-clear-error-handling-for-fetch-failures-invalid-keys-and-rate-limits"></a>

### Overview<a name="overview-3"></a>

Transparent, actionable error messages guide users to resolve issues without
confusion or silent failures.

### Key Requirements<a name="key-requirements-3"></a>
...
- **Expandable details**: "Show full error" for debugging/logs (console or user
  report)
...
- User report generation: include sanitized logs (without API keys) for bug
  reports

______________________________________________________________________

## 5. Block-Level Synchronization with Hover/Click Linkage Between Panes<a name="5-block-level-synchronization-with-hoverclick-linkage-between-panes"></a>

### Overview<a name="overview-4"></a>

Clicking or hovering over a block in one pane highlights the corresponding block
in the other pane, maintaining mental mapping as users read across languages.

### Key Requirements<a name="key-requirements-4"></a>
...
- **Hover behavior**:
  ...
  - Corresponding translation block in right pane highlights (e.g., light yellow
    background, 0.2s animation)
...
- **Click behavior**:
  ...
  - Keyboard: Tab key cycles through blocks; Enter to jump to corresponding
    block
...
- **Scroll synchronization**:
  ...
  - When user scrolls one pane, optional side-scroll locking (disabled by
...
- **Screen readers**: Announce block position and linkage ("Paragraph 3: Click
  to see translation")
...
## Phase 1 Implementation Priority<a name="phase-1-implementation-priority"></a>
...
## Phase 1 Success Metrics<a name="phase-1-success-metrics"></a>
...
# Phase 2: Translation Refinement & User Knowledge Artifacts<a name="phase-2-translation-refinement--user-knowledge-artifacts"></a>

This phase extends Luminote with re-translation capabilities, fallback modes,
content annotation, and versioning—enabling users to refine translations and
build personal knowledge assets.

______________________________________________________________________

## 1. Re-Translate Per-Block or Full Document with Custom Prompts<a name="1-re-translate-per-block-or-full-document-with-custom-prompts"></a>

### Overview<a name="overview-5"></a>

Allow users to regenerate translations at block or document level using custom
prompts, system instructions, and alternative models. Maintain version history
for comparison and rollback.

### Key Requirements<a name="key-requirements-5"></a>

#### 1.1 Re-Translation Triggers

- **Block-level re-translation**:

  - Right-click menu on translated block: "Re-translate", "Re-translate with
    custom prompt"
...
- **Prompt template modal**:
  ...
  - Pre-filled template:
    `"Translate the following {source_lang} text to {target_lang}. Consider: [custom instructions]"`
  - Text area for user instructions (e.g., "Use formal tone, preserve technical
    terms")
  - Dropdown to select system role (e.g., "Professional translator", "Academic
    translator", "Casual translator")
  - Optional: insert variables like `{source_lang}`, `{target_lang}`,
    `{context}` (surrounding blocks)

- **Quick templates**:

  - Pre-built templates: "Formal", "Casual", "Technical", "Literal",
    "Interpretive"
...
- **Alternative model picker**:
  ...
  - Model comparison mode: re-translate with Model A vs Model B, show
    side-by-side

- **Provider fallback**:

  - If primary provider fails, auto-retry with secondary provider (if
    configured)
...
- **Include context**:
  ...
  - Send surrounding blocks (previous + current + next) to model for better
    coherence
  - Toggle "context-aware" mode (default: on for paragraphs, off for individual
    sentences)
...
## 2. Local Visit History and Quick Paste-Text Translation<a name="2-local-visit-history-and-quick-paste-text-translation"></a>

### Overview<a name="overview-6"></a>

Maintain a searchable browsing history of translated URLs, and provide fallback
paste-text mode for content that cannot be extracted or fetched.

### Key Requirements<a name="key-requirements-6"></a>
...
- **History storage**:
  ...
  - Each entry: URL, title, extraction timestamp, source language, target
    language, provider/model used
  - Metadata: word count, extraction quality score (% of content successfully
    extracted)
...
- **Trash/Archive**:
  ...
  - Tag entries for personal organization (e.g., "research", "reading-list",
    "references")
...
- **Paste interface**:
  ...
  - Character counter: show current/max length (max configurable, e.g., 50k
    chars)
...
## 3. Selection-Based Commands: Explain, Define, Summarize (Save as Notes)<a name="3-selection-based-commands-explain-define-summarize-save-as-notes"></a>

### Overview<a name="overview-7"></a>

Enable users to select text in either pane and trigger context-specific AI
operations to build understanding and annotate content.

### Key Requirements<a name="key-requirements-7"></a>
...
- **Selection mechanism**:
  ...
  - Menu includes: Explain, Define, Summarize, Add Note (+ extensibility for
    custom commands)
...
- **Behavior**:
  ...
  - Explanation includes: definition, usage examples, related concepts, context
    from document

- **Prompt template**:

  - "Explain the following term/concept in {target_lang}, providing: 1)
    Definition, 2) Two usage examples, 3) Related concepts. Context:
    \[surrounding 1-2 sentences\]"
  - Customizable system role: "English teacher", "Technical expert", "Domain
    specialist"
...
- **Prompt template**:
  ...
  - "Define this term concisely (1-2 sentences) in {target_lang}: \[selected
    text\]"
...
- **Behavior**:
  ...
  - Output length: configurable (abstract: 1 sentence, brief: 3-5 sentences,
    standard: 1 paragraph)

- **Prompt template**:

  - "Summarize the following text in {target_lang} in {length} sentences,
    focusing on key ideas: \[selected text\]"
...
## 4. Prompt Templates and Termbase Setup; Translation Versioning<a name="4-prompt-templates-and-termbase-setup-translation-versioning"></a>

### Overview<a name="overview-8"></a>

Provide tools for managing reusable prompt templates, maintaining terminology
consistency, and tracking translation history for comparison and refinement.

### Key Requirements<a name="key-requirements-8"></a>
...
- **Template structure**:
  ...
  - Template body with variables: `{source_lang}`, `{target_lang}`, `{text}`,
    `{context}`, `{tone}`, `{custom_vars}`
  - System role (persona)
...
- **Enforcement**:
  ...
  - Highlight terms from termbase in source (left pane) and translation (right
    pane)
  - If term appears in source but translation doesn't match termbase → show
    warning
  - Option to auto-correct or manually review

- **Termbase injection into prompts**:

  - Include glossary excerpt in translation prompt: "Use these terms: \[relevant
    entries\]"
...
- **Version storage**:
  ...
  - Version metadata: timestamp, model used, provider, template/prompt, language
    pair, user notes
...
## Phase 2 Implementation Priority<a name="phase-2-implementation-priority"></a>
...
1. **Should-Have** (Polish & Completeness):
...
1. **Nice-to-Have** (Future Enhancement):
...
## Phase 2 Success Metrics<a name="phase-2-success-metrics"></a>
...
- **Termbase**:
  ...
  - Terminology consistency improvement: measurable reduction in variant
    translations

______________________________________________________________________

# Phase 3: AI Insights, Verification & Knowledge Integration<a name="phase-3-ai-insights-verification--knowledge-integration"></a>

This phase transforms Luminote into a comprehensive knowledge workbench with
on-demand AI insights, multi-model verification, highlights/annotations, and
web-browsing capabilities with citations.

______________________________________________________________________

## 1. On-Demand AI Insights and Verification Packs<a name="1-on-demand-ai-insights-and-verification-packs"></a>

### Overview<a name="overview-9"></a>

Provide users with advanced analysis capabilities: claims verification, internal
consistency checking, argument extraction, and bias detection. Package these as
reusable "packs" that apply multiple AI operations in sequence.

### Key Requirements<a name="key-requirements-9"></a>
...
- **Prompt templates**:
  ...
  - "Extract all factual claims from this text: \[content\]. For each claim,
    assess: 1) Verifiability (high/medium/low), 2) Common knowledge (yes/no), 3)
    Contested status (consensus/disputed/novel)"
  - "Identify the source/authority cited for this claim: \[claim\]. Is the
    source reliable?"

- **Output**:

  - List of claims with metadata: text, type (quantitative, qualitative),
    verifiability, source
  - Color-coded annotations in translated pane (green: verifiable, yellow:
    uncertain, red: contested/false)
  - Each claim linked to source location in document
  - Suggestions for fact-checking resources (Snopes, PubMed, scholar.google.com,
    etc.)
...
- **Prompt template**:
  ...
  - "Analyze this document for internal consistency. Report: 1) Contradictions
    between claims, 2) Unresolved tensions, 3) Implicit assumptions, 4) Terms
    used inconsistently"
...
- **Prompt template**:
  ...
  - "Analyze the argument structure of this text: 1) Main thesis, 2) Key
    premises, 3) Evidence provided, 4) Logical flow, 5) Potential fallacies or
    weak points"
...
- **UI**:
  ...
  - Graphical argument map panel (nodes = premises/conclusion, edges =
    inference)
...
- **Prompt template**:
  ...
  - "Analyze this text for: 1) Bias indicators (language, framing, implicit
    assumptions), 2) Tone (neutral/opinionated/emotional/inflammatory), 3)
    Author's apparent perspective, 4) Loaded or charged language"
...
- **Prompt template**:
  ...
  - "Provide: 1) Executive summary (2-3 sentences), 2) Key takeaways (5 bullet
    points), 3) Questions answered by this document, 4) Gaps or unanswered
    questions"
...
- **Cost estimation**:
  ...
  - Auto-select tier (fast/balanced/quality) based on user preference in
    settings
...
- **Async execution**:
  ...
  - Partial results: show claims as they're extracted, rather than waiting for
    all

______________________________________________________________________

## 2. Highlights, Notes, and Saved AI Explanations<a name="2-highlights-notes-and-saved-ai-explanations"></a>

### Overview<a name="overview-10"></a>

Enable persistent annotation of documents: user-created highlights, notes, and
AI-generated explanations grouped for easy review and export.

### Key Requirements<a name="key-requirements-10"></a>
...
- **Highlight types**:
  ...
  - AI highlight: generated by insight packs (e.g., claims verification
...
- **Auto-save explanations**:
  ...
  - When user runs Explain/Define on selection (Phase 2 feature), result is
    auto-saved
...
## 3. Link Cards with Bilingual Summaries<a name="3-link-cards-with-bilingual-summaries"></a>

### Overview<a name="overview-11"></a>

For documents that contain links, generate bilingual summary cards that provide
context and encourage exploration of related resources without leaving the
workbench.

### Key Requirements<a name="key-requirements-11"></a>
...
- **Link collection**:
  ...
  - Detect link type: news article, research paper, product page, social media,
    etc.
...
- **Prompt template for translation**:
  ...
  - "Translate this title and description to {target_lang} briefly: Title:
    \[title\], Description: \[description\]. Preserve meaning but adapt for
    fluent readability."
...
## 4. Multi-Model Cross-Check and Refinement (Enhanced Mode)<a name="4-multi-model-cross-check-and-refinement-enhanced-mode"></a>

### Overview<a name="overview-12"></a>

Enable power users to query multiple AI models in parallel, compare responses,
and synthesize refined outputs—achieving higher confidence and accuracy through
model diversity.

### Key Requirements<a name="key-requirements-12"></a>
...
- **Refinement workflow**:
  ...
  1. Selects best phrases from each version
  1. Or uses "Synthesize" button to auto-merge optimal passages
  1. Edit synthetic version manually
...
- **Comparison panel**:
  ...
  - Tabs for Translation | Claims | Consistency | Bias (whatever insights were
    run)
...
## 5. Web-Browsing and RAG (Retrieval-Augmented Generation) with Citations<a name="5-web-browsing-and-rag-retrieval-augmented-generation-with-citations"></a>

### Overview<a name="overview-13"></a>

For insight packs (especially claims verification), enable AI models to search
the web and reference external sources, providing citations for verification
results.

### Key Requirements<a name="key-requirements-13"></a>
...
- **RAG pipeline**:
  ...
  1. Fetch top N results (5–10) from search provider
  1. Extract relevant snippets from results
  1. Prompt AI model to synthesize answer using search results
  1. Model provides answer with citations (e.g., "According to \[Source 1\],
     ...")
...
## 6. Integration of Phase 3 Features<a name="6-integration-of-phase-3-features"></a>

### Overview<a name="overview-14"></a>
...
1. Refines translation with custom prompts (Phase 2)
1. Saves annotations and notes (Phase 3)
1. Runs insight packs: claims verification + bias detection (Phase 3)
1. Web-search uncertain claims, add citations to notes (Phase 3)
1. Compare with alternative model translation (Enhanced Mode)
1. Export final document + annotations + bibliography (Phase 3)

#### 6.2 Research Paper Review Workflow

1. Extract and translate research paper
1. Highlight key claims and methodologies
1. Run insight pack: argument structure + claims verification
1. Web-search citations mentioned in paper
1. Generate reading list from linked references
1. Compare abstract translated by multiple models
1. Export annotated translation for future reference

#### 6.3 News Analysis Workflow

1. Translate news article
1. Run bias & tone detection
1. Extract claims and verify with web search
1. Identify key takeaways
1. Compare article with translations from other sources (link cards)
1. Highlight contradictions and consensus points
1. Save to reading list with personal annotations

______________________________________________________________________

## Phase 3 Implementation Priority<a name="phase-3-implementation-priority"></a>
...
## Phase 3 Success Metrics<a name="phase-3-success-metrics"></a>
...
- **Backend:** Python 3.12+ with FastAPI (content fetch, extraction, translation
  orchestration)
- **Frontend:** Node.js 22+ with SvelteKit and TypeScript (dual-pane UI, block
  synchronization, settings management)
- **Storage:** Client-side (browser localStorage/IndexedDB) for config and local
  history
...
### Core Abstractions<a name="core-abstractions"></a>
